...
<!--                 <dd changepirce="0" class="active">{{saleAttrValueName}}</dd>
  ...
  export default {
    ...
    methods: {
      ...
      async addShopcar() {
        ...
        // 调用vuex仓库中的addOrUpdateShopCart函数
        // 2.需要知道这次请求成功还是失败，如果成功则进行路由跳转，如果失败，需要给用户提示
        try{
          // 成功
          await this.$store.dispatch('addOrUpdateShopCart', {skuId: this.$route.params.skuid, skuNum: this.skuNum})
          // 进行路由跳转
          // 在路由跳转的时候还需要将产品的信息带给下一级的路由组件
          // 一些简单的数据skyNum,通过query形式给路由组件传递过去
