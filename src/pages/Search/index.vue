...
export default {
  ...
  computed: {
    // mapGetters里面的写法：传递的数组，因为getters计算式没有划分模块【】
  ...
  methods: {
    ...
    removeCategoryName() {
      ...
      // 带给服务器参数说明可有可无的，如果属性值为空的字符串还是回吧相应的字段带给服务器
    ...
    // 修改Order属性
    changeOrder(flag) {
      // flag形参：是一个标记，代表用户点击的是综合（1）价格（2）【用户点击的时候传递进来】
...
/*    
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      ...
      // 再次发起ajax请求
      this.getData();
      // 每一次请求完毕，应该把相应的1，2，3级分类的id置空的，让他接收下一次的相应1、2、3id
      // 分类的名字与关键字不用清理，因为每一次路由发生变化的时候，都会给它赋予新的数据
      this.searchParams.category1Id = "";
      this.searchParams.category2Id = "";
      this.searchParams.category3Id = "";
    },
